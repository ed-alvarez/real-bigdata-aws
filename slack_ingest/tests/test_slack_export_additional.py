...
INDEX_NAME = "slack-parse-v1-000001"  # Aliases only support one underlying index for .get, .delete etc
INDEX_NAME = (
    "slack-parse-v1-2021-03"  # Aliases only support one underlying index for .get, .delete etc. So have to use the underlying index.
)
...
# Additional slack export test for full test run
def test_slack_export_download_1():
    ...
    # assert (
    #     data["channels"]
    #     == f"dev.archived.slack/{todays_y_m_d}/json-slack/channels.json"
    # )
    assert "dev.archived.slack" in data["channels"]
    assert "json-slack/channels.json" in data["channels"]
    # assert data["users"] == f"dev.archived.slack/{todays_y_m_d}/json-slack/users.json"
    assert "dev.archived.slack" in data["users"]
    assert "json-slack/users.json" in data["users"]
    ...
    assert source["item_id"]["es_id"] == "ips-G01QGS2FY1J-210317-0"


# Re run March 18th this time with no slack api token (monkeypatched out)
def test_slack_export_download_2_no_api_token(monkeypatch):
    def monkeypatch_try_get_slack_token(client_name):
        return ""

    monkeypatch.setattr(settings, "try_get_slack_api_token", monkeypatch_try_get_slack_token)
    sat = settings.try_get_slack_api_token(CLIENT_NAME)
    print(f"Settings Slack API Token for {CLIENT_NAME}:__{sat}__")
    assert sat == ""
    ...
    attachments = [
        ...
        # "dev.processed.slack/2021-03-18/attachments/F01S4DQJP5X.zip", we can no longer test this scenario of shared priv channel because fing-su free trial of slack connect expired.
    ]
    ...
    for msg, msg_count in zip(
        [
            ...
            shared_channel,
            normal_channel,
            # connect trial expired direct_message_with_slack_connect,
        ],
        msg_counts,
    ):
