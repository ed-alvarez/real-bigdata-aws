...
class DynamoClient:
    def __init__(self, customer_name, ddb_client=None):
        self.dynamo_client = ddb_client or boto3.client("dynamodb")
        self.customer: str = customer_name
    ...
    def create_table(
        ...
        try:
            ...
            error_msg = (
                f"Couldn't create table {table_name}. Here's why: {err.response['Error']['Code']}:{err.response['Error']['Message']}"
            )
    ...
    def get_table_information(self, table_name: str) -> bool:
        ...
        try:
            ...
            if err.response["Error"]["Code"] == "ResourceNotFoundException":
                logger.debug(
                    f"Error getting information of {table_name}. Here's why: {err.response['Error']['Code']}: {err.response['Error']['Message']}"
                )
            else:
                logger.debug(
    ...
    def table_query(
        ...
        if key_condition_expression and expression_attribute_values:
            try:
                ...
                if err.response["Error"]["Code"] == "ResourceNotFoundException":
                    logger.debug(
                        f"Error executing the query. Here's why: {err.response['Error']['Code']}: {err.response['Error']['Message']}"
                    )
                else:
                    logger.debug(
                        f"Couldn't check for existence of {table_name}. Here's why: {err.response['Error']['Code']}: {err.response['Error']['Message']}"
    ...
    def put_item(self, table_name: str, item: dict) -> bool:
        ...
        try:
            ...
            if err.response["Error"]["Code"] == "ResourceNotFoundException":
                logger.debug(
                    f"Error putting item in {table_name}. Here's why: {err.response['Error']['Code']}: {err.response['Error']['Message']}"
                )
            else:
                logger.debug(
                    f"Couldn't check for existence of {table_name}. Here's why: {err.response['Error']['Code']}: {err.response['Error']['Message']}"
            ...
            logger.debug(
                f"Couldn't put the item into the table {table_name}. Here's why: {err.response['Error']['Code']}: {err.response['Error']['Message']}"
            )
    ...
    def get_item(self, table_name: str, key: dict):
        ...
        try:
            result = self.dynamo_client.get_item(TableName=table_name, Key=key)
            ...
            logger.debug(
                f"Couldn't get item from the table {table_name}. Here's why: {err.response['Error']['Code']}: {err.response['Error']['Message']}"
            )
            return None
